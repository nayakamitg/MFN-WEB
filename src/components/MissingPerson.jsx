...
const MissingPerson = () => {
  // We'll use missingloading from Redux instead of local loading state
  ...
  // Using filter state from Redux instead of local state
  // const [visible, setVisible] = useState(false);
  ...
  const { missingPersons, missingloading, missingerror } = useSelector(
    (state) => state.missing
  );
...
console.log("Missing=",missingPersons)
  useEffect(() => {
    if (page > 1) {
      dispatch(getMissingPersons({ page, size: 20, data:filter }));
    }
  }, [page]);
  ...
  useEffect(() => {
    ...
    dispatch(getMissingPersons({ page: 1, size: 20, data:filter }));
  }, [filter]);
  ...
  const handleReset = () => {
    ...
    // Reset form elements - no need to manually reset since we're using controlled components
    // document.querySelectorAll('form').forEach(form => form.reset());
   let formElements=document.querySelectorAll('form,input[type="search"],select')
...
<label htmlFor="" >Age Group</label>
         ...
         <div className="FilterOne">
         <p 
           className={`FilterTitle ${filter.ageGroup === "child" ? "selected" : ""}`}
           onClick={() => {
           ...
           style={filter.ageGroup === "child" ? { backgroundColor: "#2563eb", color: "white", border: "none" } : {}}
         >
           Child (0-12) {filter.ageGroup === "child" && <Check size={16} />}
         </p>
         <p 
           className={`FilterTitle ${filter.ageGroup === "teen" ? "selected" : ""}`}
           onClick={() => {
             dispatchFilter(setFilter({
               ageGroup: "teen",
               AgeFrom: 13,
           ...
           style={filter.ageGroup === "teen" ? { backgroundColor: "#2563eb", color: "white", border: "none" } : {}}
         >
           Teen (13-19) {filter.ageGroup === "teen" && <Check size={16} />}
         </p>
         <p 
           className={`FilterTitle ${filter.ageGroup === "adult" ? "selected" : ""}`}
           onClick={() => {
             dispatchFilter(setFilter({
               ageGroup: "adult",
               AgeFrom: 20,
           ...
           style={filter.ageGroup === "adult" ? { backgroundColor: "#2563eb", color: "white", border: "none" } : {}}
         >
           Adult (20-59) {filter.ageGroup === "adult" && <Check size={16} />}
         </p>
         <p 
           className={`FilterTitle ${filter.ageGroup === "senior" ? "selected" : ""}`}
           onClick={() => {
             dispatchFilter(setFilter({
               ageGroup: "senior",
               AgeFrom: 60
           ...
           style={filter.ageGroup === "senior" ? { backgroundColor: "#2563eb", color: "white", border: "none" } : {}}
         >
           Senior (60+) {filter.ageGroup === "senior" && <Check size={16} />}
         </p>
         {filter.ageGroup && (
           <p 
             className="FilterTitle"
             style={{ backgroundColor: "#fee2e2", color: "#dc2626", cursor: "pointer" }}
             onClick={() => {
               dispatchFilter(setFilter({
                 ageGroup: "",
                 AgeFrom: "",
...
<label htmlFor="" >Time Period</label>
         ...
         <div className="FilterOne">
         <p 
           className={`FilterTitle ${filter.timePeriod === "week" ? "selected" : ""}`}
           onClick={() => {
           ...
           style={filter.timePeriod === "week" ? { backgroundColor: "#2563eb", color: "white", border: "none" } : {}}
         >
           This Week {filter.timePeriod === "week" && <Check size={16} />}
         </p>
         <p 
           className={`FilterTitle ${filter.timePeriod === "month" ? "selected" : ""}`}
           onClick={() => {
             const today = new Date();
           ...
           style={filter.timePeriod === "month" ? { backgroundColor: "#2563eb", color: "white", border: "none" } : {}}
         >
           This Month {filter.timePeriod === "month" && <Check size={16} />}
         </p>
         <p 
           className={`FilterTitle ${filter.timePeriod === "year" ? "selected" : ""}`}
           onClick={() => {
             const today = new Date();
           ...
           style={filter.timePeriod === "year" ? { backgroundColor: "#2563eb", color: "white", border: "none" } : {}}
         >
           This Year {filter.timePeriod === "year" && <Check size={16} />}
         </p>
          ...
          <div className="searchContainer">
            <div className="SearchFilter">
              ...
              <select
                ...
                value={filter.gender}
                onChange={(e) => dispatchFilter(setFilter({ gender: e.target.value }))}
              >
        ...
        <div className="missingPersonCards">
          {missingPersons.data.length > 0
            ? missingPersons?.data?.map((person, index) => (
                <Card personType="missing" key={index} person={person} />
              ))
            : !missingloading && (
...
<div className="Pagination">
    <button disabled={!missingPersons.hasPreviousPage} onClick={()=>{setPage(page-1)}} className="prev"><ArrowLeft/></button>
    <span style={{color:"black"}}>{missingPersons.pageNumber}</span>
    <span>...</span>
    <span>{missingPersons.totalPages}</span>
    <button disabled={!missingPersons.hasNextPage} onClick={()=>{setPage(page+1)}} className="next"><ArrowRight/></button>
</div> */}
      </div>
      <button
