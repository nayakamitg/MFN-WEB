...
const UnidentifiedDeadBodiesDetails = () => {
  ...
  const getPersons = async (id) => {
    try {
      ...
      const res = await axios.get(
        `https://goplanup.dishaayein.com/api/UnIdentifiedDeadBodies/${id}`
      );
      setPerson(res.data);
      setLoading(false)
    } catch (error) {
...
if(loading){
  ...
  return (
    ...
    <div className="descriptionContainer">
        <h1 className="nameTitle">{person?.name || "UNKNOWN"}</h1>
     ...
     <thead>
      ...
      <tbody>
       ...
       { person?.policePost && <tr>
          <td width={50+"%"}>Police Post</td>
          <td>{person.policePost}</td>
        </tr>}
     ...
     <thead>
      ...
      <tbody>
       ...
       { person?.postMortemResult && <tr>
          <td width={50+"%"}>Post Mortem Result</td>
          <td>{person?.postMortemResult}</td>
        </tr>}
...
export default UnidentifiedDeadBodiesDetails
